...
if ./poisson -n 7 -i 300 \
    -s reference/asymmetrical-source-coordinates.txt \
    | cmp reference/7-asym.txt; then
    echo "n=7 i=300 (asymmetrical source) correct"
else
    echo "n=7 i=300 (asymmetrical source) failed!"
    status=1
fi
